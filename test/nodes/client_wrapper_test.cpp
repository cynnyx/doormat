...
/** ****************************************************************************************
 * // TODO: remove this when the chain has been moved to cloudia
 */
inline node_interface::data_t make_chunk_ptr(const std::string& s) {
	auto ptr = std::make_unique<char[]>(s.size());
	std::copy(s.begin(), s.end(), ptr.get());
	return std::move(ptr);
}

inline bool equal_body_parts(const std::pair<node_interface::data_t, size_t>& p1,
							 const std::pair<node_interface::data_t, size_t>& p2) {
	return std::equal(p1.first.get(), p1.first.get() + p1.second,
					  p2.first.get(), p2.first.get() + p2.second);
}

inline bool equal_body_parts(const std::pair<node_interface::data_t, size_t>& p1, const std::string& p2) {
	return equal_body_parts(p1, std::make_pair(make_chunk_ptr(p2), p2.size()));
}
...
TEST_F(client_wrapper_test, preamble_only)
{
	...
	auto init_fn = [this, submitted_req, expected_bytes](boost::asio::io_service& ios) mutable
	{
		...
		ch = make_shared_chain<node_interface, first_node, nodes::client_wrapper, blocked_node>();
		stop_condition = [this](){ return true; };
		server.reset( new mock_server<>{service::locator::service_pool().get_thread_io_service(), port} );
...
TEST_F(client_wrapper_test, custom_destination_ok)
{
	...
	submitted_req.keepalive(false);
	submitted_req.header(http::hf_cyn_dest, "127.0.0.1");
	submitted_req.header(http::hf_cyn_dest_port, std::to_string(port));
	submitted_req.setParameter("hostname", "::");
	submitted_req.setParameter("port", std::to_string(port));
	auto d1 = submitted_req.serialize();
	const auto expected_bytes = d1.size();
...
TEST_F(client_wrapper_test, complete_request)
{
	...
	auto init_fn = [this, submitted_req, expected_bytes](auto&&) mutable
	{
		preset::init_thread_local();
		ch = make_shared_chain<node_interface, first_node, nodes::client_wrapper, blocked_node>();
		...
		ch->on_request_preamble(std::move(submitted_req));
		ch->on_request_body(make_chunk_ptr(body), body.size());
	};
...
TEST_F(client_wrapper_test, no_response)
{
	...
	auto init_fn = [this, submitted_req](auto&&) mutable
	{
		preset::init_thread_local();
		ch = make_shared_chain<node_interface, first_node, nodes::client_wrapper, blocked_node>();
		stop_condition = [this](){
...
TEST_F(client_wrapper_test, request_response)
{
	...
	auto init_fn = [this, submitted_req, expected_bytes, &res_d](auto&&) mutable
	{
		preset::init_thread_local();
		ch = make_shared_chain<node_interface, first_node, nodes::client_wrapper, blocked_node>();
...
TEST_F(client_wrapper_test, server_hangs)
{
	...
	auto res_d = submitted_res.serialize();
	auto init_fn = [this, submitted_req, &res_d](auto&&) mutable
	{
		preset::init_thread_local();
		ch = make_shared_chain<node_interface, first_node, nodes::client_wrapper, blocked_node>();
